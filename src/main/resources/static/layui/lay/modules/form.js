            ...
            select: function(){
                            ...
                            if (isMulti) {
                                ...
                                if(othis.find("input[type='checkbox']").is(':checked')){
                                    ...
                                    if (omit) {
                                        ...
                                        if(othis.parent().find('[type=checkbox]').length == othis.parent().find('[type=checkbox]:checked').length){
                                            if(othis.parent().find('[type=checkbox]').length==1){
                                                input.eq(0).val(valueStr);
                                            }else {
                                                input.eq(0).val("ALL");
                                            }
                                        }else {
                                            if(othis.parent().find('[type=checkbox]:checked').length==1){
                                                input.eq(0).val(valueStr);
                                            }else {
                                                input.eq(0).val(othis.parent().find('[type=checkbox]:checked').length+" items");
                                            }
                                        }
                ...
                selects.each(function(index, select){
                    ...
                    var isSearch = typeof othis.attr('lay-search') === 'string'
                        ...
                        ,inputValue =  !(typeof $(select).attr("lay-omit") === 'undefined')&&value!=null&&value.length>1 ? value.length+" items" : value;
                    //乔永宝替换2018-11-21
                    // ,inputValue =  !(typeof $(select).attr("lay-omit") === 'undefined')&&value!=null&&value.length>0 ? value.length+" items" : "";
                    ...
                    if (isMulti) {
                        var reElem = $(['<div class="' + (isMulti ? '' : 'layui-unselect ') + CLASS + (disabled ? ' layui-select-disabled' : '') + '">'
                            ...
                            , '<dl class="layui-anim layui-anim-upbit' + (othis.find('optgroup')[0] ? ' layui-select-group' : '') + '" '+ ((isSearch || isTools)?'style="overflow-y: hidden;"':'') +'>' + function (options) {
                                var arr = [], height=247 ,tools = '<dd class="layui-select-tips"><div class="multiOption" data-value="all"><i class="iconfont icon-quanxuan"></i>ALL</div><div class="multiOption" data-value="none"><i class="iconfont icon-qingkong"></i>Empty</div><div class="multiOption" data-value="inverse"><i class="iconfont icon-fanxuan"></i>Inverse</div></dd>';
                                layui.each(options, function (index, item) {
                                    if (index === 0 && !item.value) {
                                        if (isSearch) {
                                            arr.push('<dd lay-value="" class="layui-select-tips" style="padding-right: 10px; margin-bottom: 5px;"><input class="layui-input" placeholder="Keyword Search"></dd>');
                                            if (isTools) {
                                                arr.push(tools);
                                                height -= 37;
                                            }
